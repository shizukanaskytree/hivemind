...
class CollaborativeOptimizer(DecentralizedOptimizerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.target_batch_size, self.batch_size_per_step = (
            target_batch_size,
            batch_size_per_step,
        )
        ...
        self.expected_drift_peers, self.expected_drift_rate = (
            expected_drift_peers,
            expected_drift_rate,
        )

        self.averaging_timeout, self.metadata_expiration = (
            averaging_timeout,
            metadata_expiration,
        )

        self._grads, self.reuse_grad_buffers, self.accumulate_grads_on = (
            None,
            reuse_grad_buffers,
            accumulate_grads_on,
        )
        ...
        # a number of local samples accumulated since last optimizer update
        self.local_samples_accumulated = 0
        # a number of calls to step() since last optimizer update
        self.local_steps_accumulated = 0
        self.performance_ema = PerformanceEMA(alpha=performance_ema_alpha)
        self.last_step_time = None

        self.collaboration_state = self.fetch_collaboration_state()
        self.lock_collaboration_state, self.collaboration_state_updated = (
            Lock(),
            Event(),
        )
        ...
        self.collaboration_state_updater = Thread(
            target=self.check_collaboration_state_periodically,
            daemon=True,
    ...
    def local_step(self) -> int:
        """
        global optimizer step depends on how you define a step
        within which how many samples are there.
        """
    ...
    def is_synchronized(self) -> bool:
        return (
            self.local_step
            >= self.collaboration_state.optimizer_step - self.step_tolerance
        )
    ...
    def step(self, batch_size: Optional[int] = None, **kwargs):
        ...
        if self.batch_size_per_step is None:
            if batch_size is None:
                raise ValueError(
                    "Please either set batch_size_per_step parameter at init or when calling .step"
                )
            logger.log(
                self.status_loglevel,
                f"Setting default batch_size_per_step to {batch_size}",
            )
        ...
        if (
            ...
            and get_dht_time() - self.last_step_time > self.metadata_expiration
        ):
            logger.warning(
        ...
        with self.performance_ema.pause(), self.lock_collaboration_state:
            ...
            if self.collaboration_state.num_peers > 1:
                mean_samples_per_worker = (
                    self.target_batch_size / self.collaboration_state.num_peers
                )
                weight = self.local_samples_accumulated / mean_samples_per_worker
                try:
                    group_info = self.averager.step(
                        weight=weight, timeout=self.averaging_timeout, **kwargs
                    )
                    if group_info:
                        logger.log(
                            self.status_loglevel,
                    ...
                    logger.log(
                        self.status_loglevel,
                        f"Skipped averaging: averaging round failed with {repr(e)}.",
                    )
            ...
            self.collaboration_state_updated.set()
            # scheduler update: global optimizer steps based
            # e.g., Collaboration accumulated 312 samples then we define it as a global step.
            self.update_scheduler()
    ...
    def step_aux(self, **kwargs):
        ...
        with self.lock_collaboration_state:
            ...
            try:
                group_info = self.averager.step(
                    timeout=self.averaging_timeout, **kwargs
                )
                if group_info:
                    logger.log(
                        self.status_loglevel,
    ...
    def accumulated_grads(self) -> Iterator[torch.Tensor]:
        ...
        if self.reuse_grad_buffers:
            ...
            with torch.no_grad():
                self._grads = [
                    torch.zeros_like(grad, device=self.accumulate_grads_on)
    ...
    def fetch_collaboration_state(self) -> CollaborationState:
        ...
        response, _expiration = self.dht.get(
            self.training_progress_key, latest=True
        ) or (None, -float("inf"))
        current_time = get_dht_time()
        ...
        if not isinstance(response, dict) or len(response) == 0:
            ...
            local_eta_next_step = (
                max(0, self.target_batch_size - self.local_steps_accumulated)
        ...
        expected_max_peers = max(
            num_peers + self.expected_drift_peers,
            num_peers * (1 + self.expected_drift_rate),
        )
    ...
    def update_scheduler(self):
        if self.scheduler:
            # print("update_scheduler?", self.scheduler._step_count < self.local_step)
            while self.scheduler._step_count < self.local_step:
                # print("self.scheduler._step_count", self.scheduler._step_count)
                # print("self.local_step", self.local_step)
                self.scheduler.step()
