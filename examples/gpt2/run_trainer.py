...
class CollaborativeCall:
    def __init__(
        ...
        self.samples = 0
        # self.steps is used to average the training loss
        self.steps = 0
        self.loss = 0
...
class dialogpt:
    ...
    def tokenize_raw_data(self):
        ...
        raw_list = raw_data.progress_apply(
            lambda x: self.tokenizer_sen(x["question"])
            + self.tokenizer_sen(x["response"])[1:],
            axis=1,
        )
...
def main():
    ...
    collaborative_call = CollaborativeCall(
        dht=dht,
        optimizer=collaborative_optimizer,
        model=dialo.get_model(),
